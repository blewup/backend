...
router.post('/', [authenticateToken, validate(allianceValidation.create)], async (req, res, next) => {
  try {
    const {
      name,
      tag,
...
router.put('/:id', [authenticateToken, validate(allianceValidation.update)], async (req, res, next) => {
  try {
    const alliance = await db.Alliance.findByPk(req.params.id);
...
router.delete('/:id', [authenticateToken, validate(allianceValidation.delete)], async (req, res, next) => {
  try {
    const alliance = await db.Alliance.findByPk(req.params.id);
    ...
    if (alliance.leader_id !== req.userId) {
      const error = new Error('Only alliance leader can disband the alliance');
      error.status = 403;
      throw error;
    }
